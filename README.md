...
Each release page publishes two SPDX 2.3 JSON documents as plain,
downloadable assets — no `cosign` required just to read them:

| Asset | Covers | Signed via |
|---|---|---|
| `collector-<tag>-image.spdx.json` | The pushed container image (generated by [`anchore/sbom-action`](https://github.com/anchore/sbom-action) against the image digest) | `cosign attest` on that same digest — verify with the `cosign verify-attestation` command above |
