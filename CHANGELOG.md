...
- **SBOM generation wired into `release.yml`**, closing the `[0.2.0]`
  follow-up below. Two SPDX 2.3 JSON documents are published per release:
  - `collector-<tag>-image.spdx.json`, generated by `anchore/sbom-action`
    against the pushed container image digest and attested to that digest
    with `cosign attest --type spdxjson` (keyless, same OIDC identity as
    image/binary signing).
  - `collector-<tag>-source.spdx.json`, one document for the Go module
    graph shared by all four platform binaries (they are `CGO_ENABLED=0`
    static builds of the same `go.mod`/`go.sum`, so a single source-level
    SBOM covers `linux/amd64`, `linux/arm64`, `darwin/amd64`, and
    `darwin/arm64` without a redundant per-arch scan). Signed as a blob
    with `cosign sign-blob --bundle`, matching the existing binary-signing
    pattern.

  Both documents are also plain downloadable release assets — no `cosign`
  required just to read them. See README's new "Verifying release
  artifacts" section for the verification commands, which also documents
  the previously-undocumented `cosign verify` / `cosign verify-blob`
  commands for the image and binaries.
...
- **SBOM generation is not wired into `release.yml`.** No `syft`/
  ...
  as a follow-up, not bundled in here. **Done — see `[Unreleased]` above.**
